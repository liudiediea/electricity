  ...
  <div id="brand_style">
	...
	<tbody>
       ...
       <tr>
          ...
          <td>{{$v->id}}</td>
          <td><img src="{{Storage::url($v['logo']) }}"  width="130"/></td>
          <td><u style="cursor:pointer" class="text-primary brond_name" onclick="member_show('张三','member-show.html','10001','360','400')">{{$v->name}}</u></td>
          <td>{{$v->address}}</td>
          ...
          <a title="删除" href="{{route('brand_del',['id'=>$v->id]) }}" onclick="return confirm('确定要删除吗？')" class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120"></i></a>
          </td>
		</tr>
       @endforeach
        </tbody>
